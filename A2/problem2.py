...
class NQueenState(GAState):
    ...
    def mutate(self):
        ...
        self.genes[random.randint(0,len(self.genes) - 1)] = random.randint(0,len(self.genes) - 1)
...
class NQueenProblem(Problem):
    ...
    def actions(self, state):
        ...
        for x in range(0, self.N):
            if state.genes[x] != 0:
                newState = NQueenState(state.genes.copy())
                newState.genes[x] = state.genes[x] - 1
                states.append(newState)
            if state.genes[x] != self.N - 1:
                newState = NQueenState(state.genes.copy())
                newState.genes[x] = state.genes[x] + 1
                states.append(newState)
        return states
    ...
    def value(self, state):
        ...
        for i in range(0, self.N):
            for j in range(i+1, self.N):
                if not self.conflict(state.genes[i], i, state.genes[j], j):
                    nonConflicts = nonConflicts + 1
        return nonConflicts
...
def generateStartState():
    n = 10
    state = NQueenState([random.randint(0, n) for x in range(n)])
    return state
...
def main():
    with open('output.txt', 'w') as f:
        for i in range(10):
            ...
            gp = NQueenProblem(10, state)
            goal = genetic_search(gp, NQueenProblem.value, ngen=200, pmut=0.1)
            f.write("Final State: " + str(goal[0].genes) + ' ')
            f.write("Highest Achieved: " + str(goal[1]) + ' ')
            f.write("Generation Optimal State reached at: " + str(goal[2]) + '\n \n')
    print("File output.txt has been closed: " + str(f.closed))
